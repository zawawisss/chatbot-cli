...
export class AgenticInteractiveMode extends InteractiveMode {
    agenticBot;
    agenticOptions;
    constructor(agenticBot, config, options = {}) {
        super(config);
        this.agenticOptions = {
            useAgenticLoop: true,
            maxTurns: 10,
    ...
    async handleAgenticCommands(input) {
        if (!input.startsWith('/')) {
            return false;
        }
