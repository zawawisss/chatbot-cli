...
export class InteractiveMode {
    ...
    async start() {
        ...
        if (!isValid) {
            UIComponents.showWarning('Current provider configuration might be invalid', 'Use the /setup command to configure providers');
        }
    ...
    async handleStreamChat(message) {
        ...
        UIComponents.showStreamingStatus(this.chatbot.getCurrentProvider(), this.chatbot.getCurrentModel(), 'connecting');
        const streamDisplay = UIComponents.createStreamDisplay('Assistant');
        try {
            streamDisplay.start();
            // Update status to streaming
            UIComponents.showStreamingStatus(this.chatbot.getCurrentProvider(), this.chatbot.getCurrentModel(), 'streaming');
            let response = '';
            let chunkCount = 0;
            ...
            UIComponents.showStreamingStatus(this.chatbot.getCurrentProvider(), this.chatbot.getCurrentModel(), 'complete');
        }
        catch (error) {
            streamDisplay.error(error instanceof Error ? error.message : String(error));
            // Show error status
            UIComponents.showStreamingStatus(this.chatbot.getCurrentProvider(), this.chatbot.getCurrentModel(), 'error');
        }
    }
