...
import { EnhancedUIComponents, ChatMessage, SessionInfo, StatusInfo } from './EnhancedUIComponents.js';
...
export class EnhancedInteractiveMode {
  ...
  private async showEnhancedWelcome(): Promise<void> {
  ...
  private async handleEnhancedCommand(input: string): Promise<void> {
    ...
    switch (command) {
      ...
      case 'stream':
        if (args.length === 0) {
          EnhancedUIComponents.showEnhancedError(
            'Usage: /stream <message>',
            ['Provide a message to get streaming response']
          );
          break;
        }
      ...
      case 'nostream':
        if (args.length === 0) {
          EnhancedUIComponents.showEnhancedError(
            'Usage: /nostream <message>',
            ['Provide a message to get non-streaming response']
          );
          break;
        }
      ...
      default:
        EnhancedUIComponents.showEnhancedError(
          ...
          ['Type /help to see available commands', 'Check your spelling']
        );
    }
  }

  private async handleEnhancedChat(message: string): Promise<void> {
  ...
  private async handleNonStreamingChat(message: string): Promise<void> {
    const startTime = Date.now();
    ...
    try {
      ...
      EnhancedUIComponents.showEnhancedError(
        ...
        ['Check your API key', 'Verify network connection', 'Try switching providers']
      );
    }
  }
  ...
  private async showProviderSwitcher(): Promise<void> {
    const providers = [
  ...
  private async switchProvider(providerName: string): Promise<void> {
    ...
    try {
      ...
      EnhancedUIComponents.showEnhancedError(
        error instanceof Error ? error.message : String(error),
        ['Check provider configuration', 'Verify API keys', 'Use /setup to configure']
      );
    }
  }

  private async showEnhancedProviders(): Promise<void> {
    const providers = [
