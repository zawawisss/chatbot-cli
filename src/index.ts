...
import { ChatBot, AgenticChatOptions } from './chatbot/ChatBot.js';
...
program
  ...
  .action(async (options) => {
    try {
      ...
      if (options.provider) config.set('provider', options.provider as ProviderType);
      if (options.model) config.set('model', options.model);
      if (options.temperature) config.set('temperature', parseFloat(options.temperature));
      if (options.system) config.set('systemPrompt', options.system);
      ...
      if (options.question) {
        ...
        const agenticConfig: AgenticChatOptions = {
  ...
  .action(async (options) => {
    try {
      ...
      Object.entries(categories).forEach(([category, categoryTools]) => {
        ...
        if ((categoryTools as any[]).length === 0) return;
        
        console.log(chalk.blue(`📁 ${category.toUpperCase()} TOOLS:`));
        (categoryTools as any[]).forEach(tool => {
...
program
  ...
  .action(async () => {
    try {
      ...
      const providers = ['gemini', 'deepseek', 'ollama'] as ProviderType[];
