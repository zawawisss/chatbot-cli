...
import { Provider, Message, ChatOptions, ChatResponse, StreamResponse, ModelInfo } from '../providers/types.js';
...
export class ChatBot {
  ...
  constructor(config: Config, agenticOptions: AgenticChatOptions = {}) {
    ...
    this.agenticTurn = new AgenticTurn(
      this.toolManager,
      this.history,
      {
  ...
  async switchProvider(providerType: string): Promise<void> {
    if (!ProviderFactory.getSupportedProviders().includes(providerType as any)) {
      throw new Error(`Unsupported provider: ${providerType}`);
    }

    this.config.set('provider', providerType as any);
    this.provider = this.createProvider();
  ...
  async *askStream(question: string, options?: ChatOptions): AsyncGenerator<string> {
...
Supported providers: ${ProviderFactory.getSupportedProviders().join(', ')}
  ...
  getAgenticStats(): { maxTurns: number; maxToolCalls: number; timeout: number } {
    return {
      maxTurns: this.agenticOptions.maxTurns!,
      maxToolCalls: this.agenticOptions.maxToolCalls!,
  ...
  private isComplexTask(question: string): boolean {
    ...
    const hasFileOps = fileOperations.some(indicator => 
      lowerQuestion.includes(indicator)
    );
    
    // Check for tool mentions
    const tools = this.toolManager.getTools();
    const hasToolMentions = tools.some(tool => 
      lowerQuestion.includes(tool.name.toLowerCase())
    );
  ...
  async askWithAgenticMode(question: string, options?: ChatOptions): Promise<string> {
  ...
  private async executeAgenticLoop(question: string, options?: ChatOptions): Promise<string> {
    const userMessage = {
      role: 'user' as const,
      content: question,
      timestamp: new Date()
    ...
    try {
      ...
      for await (const event of this.agenticTurn.executeAgenticLoop(
        question, 
        this.provider, 
        chatOptions
      )) {
...
Do NOT ask for permission - just execute what's needed to complete the task!`;
  ...
  protected async confirmToolExecution(toolCall: any): Promise<boolean> {
  ...
  async executeAgenticTask(task: string): Promise<{
